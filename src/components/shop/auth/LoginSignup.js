...
const LoginSignup = (props) => {
  ...
  return (
    <Fragment>
      ...
      >
        <div className="w-11/12 md:w-3/5 lg:w-2/4 relative space-y-4 bg-white p-10">
          {login ? <Login /> : <Signup />}

          {loginValue === "Login" ? (
            <div
              className="text-sm font-display font-semibold text-gray-700 text-center"
              onClick={(e) => changeLoginSignup()}
            >
              Have an account ?{" "}
              <a className="cursor-pointer text-indigo-600 hover:text-orange-700">
                {loginValue}
              </a>
            </div>
          ) : (
          ...
          <div className="absolute top-0 right-0 mx-4">
            <svg
              ...
              className="w-8 h-8 cursor-pointer hover:text-orange-700"
              fill="currentColor"
              viewBox="0 0 20 20"
