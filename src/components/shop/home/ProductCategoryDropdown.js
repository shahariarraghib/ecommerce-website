    ...
    <div className={`${data.categoryListDropdown ? "" : "hidden"} my-4`}>
      ...
      <div className="py-1 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {categories && categories.length > 0 ? (
              ...
              <Fragment key={index}>
                ...
                >
                  <img
                    src={item.cImage}
                    alt="pic"
                    className="w-full h-auto max-w-xs max-h-72"
          ...
          <div className="text-xl text-center my-4">No Category Found</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className={`${data.filterListDropdown ? "" : "hidden"} my-4`}>
      ...
      <div className="w-full flex flex-col">
        <div className="font-medium py-2">Price Range</div>
        <div className="flex justify-between items-center">
          <div className="flex flex-col space-y-2  w-2/3 lg:w-2/4">
            <label htmlFor="points" className="text-sm">
              <span className="font-semibold text-orange-700">{range}</span>{" "}
            </label>
            <input
              value={range}
              className="slider"
  ...
  return (
    ...
    >
      <input
        ...
        className="max-w-72 text-lg py-2 border-b border-gray-300 focus:outline-none focus:border-orange-700"
        type="text"
        placeholder="Search"
      />
